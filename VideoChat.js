let videoengager = (function () {
    ...
    clickButtonStartLabel = 'Clique para iniciar!', clickButtonStopLabel = 'Parar Vídeo', 
    ...
    const sendInteractionId = function (chatId, memberId) {
        ...
        $.ajax({
            url:
                `https://api.mypurecloud.com/api/v2/webchat/guest/conversations/${chatId}/members/${memberId}/messages`,
            type: "POST",
    ...
    const deleteConversation = function() {
      if(environment && chatId && memberId) {
        $.ajax({
          url: `https://api.mypurecloud.com/api/v2/webchat/guest/conversations/${chatId}/members/${memberId}`,
          type: "DELETE",
          beforeSend: function(xhr) {
                  xhr.setRequestHeader(
                          "Authorization",
